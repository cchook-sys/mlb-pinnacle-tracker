...
- 每 60 分鐘自動抓 Pinnacle 盤口 (已調整以節省額度)
- 每天自動抓賽果 + 結算昨日預測
"""
...
# ── Fetch Data with Params (修復 401 的關鍵) ──────────────────────────────────
async def fetch_api(endpoint: str):
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        r = await client.get(f"{BASE_URL}/{endpoint}/", params=params if endpoint == "odds" else {"apiKey": ODDS_API_KEY, "daysFrom": "1", "dateFormat": "iso"})
        return r
...
async def fetch_and_store_odds():
    try:
        ...
        if r.status_code != 200:
            log.error(f"Odds API error {r.status_code}: {r.text}")
            return
        games = r.json()
...
async def scheduler():
    while True:
        ...
        await asyncio.sleep(60 * 60) # 60 分鐘一次
